            ...
            if (!g.isTouch() || "mousedown" !== d.type) {
                ...
                if (f || h) {
                    ...
                    timeOutEvent = setTimeout(function() {
                        var ed = c.data("ed");
                        if (isEdit || (typeof ed != 'undefined' && ed === true)){
                            $.confirm({
                                title: '编辑或删除此图标',
                                content: '编辑/删除后, 该图标将无法恢复!',
                                type: 'green',
                                buttons: {   
                                    confirm: {
                                        text: "编辑",
                                        ...
                                        keys: ['enter'],
                                        action: function(){
                                            opEdit(c.data("title"), c.data("url"), c.parent().parent().attr("id"), c.data("tid"));
                                        }
                                    },
                                    somethingElse: {
                                        text: "删除",
                                        ...
                                        action: function(){
                                            delIco(c.data("tid"), c.parent().parent().attr("id"));
                                        }
                                    },
                                    cancel: {
                                        text: "取消",
                                        action: function(){
                                            this.setContentAppend('<div>Appended text after 2 seconds</div>');
                                        }
                                    }
                                }
                            });
                        } else if (isEdit === false && typeof ed != 'undefined' && ed === false){
                            $.alert({
                                title: '抱歉',
                                type: 'orange',
                                content: '该图标为系统推荐, 无法编辑!',
                                buttons: {
                                    cancel: {
                                        text: "关闭"
                                    }
